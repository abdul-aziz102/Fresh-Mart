...
export default function WhatsAppOrderButton({ orderId, totalAmount }) {
  const phone = process.env.NEXT_PUBLIC_WHATSAPP;
  if (!phone) return null;

  const message = encodeURIComponent(
    `Hi! I just placed Order #${orderId} for $${totalAmount.toFixed(2)} on your grocery store. Please confirm my order.`
  );
